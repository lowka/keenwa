        ...
        match input.expr().as_logical() {
                ...
                match condition {
                    JoinCondition::Using(using) => {
                        ...
                        if join_columns.len() == columns.len() && columns.iter().all(|c| join_columns.contains(c)) {
                            // Projection contains only columns from the join condition -> the projection is redundant.
            ...
            LogicalExpr::Aggregate(LogicalAggregate {
                ...
                if child_columns == columns {
                    // Projection contains only the columns produced by the aggregation -> the projection is redundant.
                    rewrite_inputs(&input)
                } else {
                    rewrite_inputs(&expr)
                }
            ...
            LogicalExpr::Get(LogicalGet {
                ...
                if child_columns == columns {
                    // Projection contains only the columns from the scan -> the projection is redundant.
                    input.clone()
                } else {
                    rewrite_inputs(&expr)
                }
