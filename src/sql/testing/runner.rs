...
/// ```
///
/// Options - per test case:
/// ```text
/// options:
///   my_opt: 1
/// <query-definition>
/// ```
/// Options - for all test cases in a file:
/// ```text
/// options:
///   my_opt: 1
/// ---
...
/// ---
/// ```
/// See [TestOptions] for the list of available options.
///
    ...
    T: TestCaseRunnerFactory,
{
    ...
    fn build_test_case_options(&self, test_case: &SqlTestCase) -> TestOptions {
        ...
        if let Some(test_case_options) = test_case.options.as_ref() {
            test_case_options.merge(&options)
        } else {
            options
        }
    }
...
pub struct SqlTestCase {
    ...
    pub expected_result: Result<String, TestCaseFailure>,
}
...
///
/// - decorrelate_subqueries `<bool>` - see [OperatorBuilderConfig.decorrelate_subqueries] (the default value is `false`).
/// - append_metadata `<bool>` - whether to include metadata of a query in the expected successful output or not (the default value is `false`).
/// - append_memo `<bool>` - whether to include a memo build by the optimizer in the the expected
/// successful output or not (the default value is `false`).
///
/// [OperatorBuilderConfig.decorrelate_subqueries]: (crate::operators::builder::OperatorBuilderConfig#structfield.decorrelate_subqueries)
#[derive(Debug, Clone)]
pub struct TestOptions {
    /// Whether to decorrelate subqueries or not.
    pub decorrelate_subqueries: Option<bool>,
    /// Whether to include metadata in the expected successful output or not.
    pub append_metadata: Option<bool>,
    /// Whether to include memo in the the expected successful output or not.
...
impl TestOptions {
    pub fn merge(&self, other: &TestOptions) -> TestOptions {
        TestOptions {
...
impl TestOptionsParser for DefaultTestOptionsParser {
    fn parse_options(&self, values: TestOptionsRaw) -> Result<TestOptions, String> {
        ...
        let options = TestOptions {
            decorrelate_subqueries: get_bool(&mut values_map, "decorrelate_subqueries")?,
            append_metadata: get_bool(&mut values_map, "append_metadata")?,
            append_memo: get_bool(&mut values_map, "append_memo")?,
        };
